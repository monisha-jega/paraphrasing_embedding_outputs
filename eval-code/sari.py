...
# Net SARI
def sari(hyps, refs, grounds):
    net_sari = 0
    for h, r, g in zip(hyps, refs, grounds):
        sari_score = SARIsent(r, h, [g])
        net_sari += sari_score
    net_sari = net_sari / float(len(refs))
    return net_sari
...
def main():
    ...
    with open(ref_file, 'r', encoding="utf8") as refs:
        ...
        for line in refs.readlines():
            # r = line.split()
            refs_arr.append(line)

    # Grond Truth
    ground_file = sys.argv[3]
    with open(ground_file, 'r', encoding="utf8") as grounds:
        grounds_arr = []
        for line in grounds.readlines():
            # g = line.split()
            grounds_arr.append(line)
    
    # SARI
    sari_score = sari(hyps_arr, refs_arr, grounds_arr)   
    print(sari_score)
